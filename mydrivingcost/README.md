...
**Start here:** `DEPLOY.md` to put it online, `MAINTENANCE.md` for the one number that ever needs updating, `REMAINING-ITEMS.md` for the honest list of what was deferred.
...
**Do not open `site/index.html` directly in a browser.** Root-absolute paths (`/assets/css/styles.css`) do not resolve under `file://`, so the page will render unstyled. Serve it over HTTP instead:
